...
$corner.css({left:leftCorner});$element.css({left:left,top:$this.offset().top-$element.height()-$corner.height()-18}).show();},$widget.options.delay);}},function(){$element.hide();clearTimeout(timer);});$(document).on('tap',function(){$element.hide();clearTimeout(timer);});$this.on('tap',function(event){event.stopPropagation();});}});$.widget('mage.SwatchRenderer',{options:{classes:{attributeClass:'swatch-attribute',attributeLabelClass:'swatch-attribute-label',attributeSelectedOptionLabelClass:'swatch-attribute-selected-option',attributeOptionsWrapper:'swatch-attribute-options',attributeInput:'swatch-input',optionClass:'swatch-option',selectClass:'swatch-select',moreButton:'swatch-more',loader:'swatch-option-loading'},jsonConfig:{},jsonSwatchConfig:{},selectorProduct:'.product-info-main',selectorProductPrice:'[data-role=priceBox]',mediaGallerySelector:'[data-gallery-role=gallery-placeholder]',selectorProductTile:'.product-item',numberToShow:false,onlySwatches:false,enableControlLabel:true,controlLabelId:'',moreButtonText:'<span class="swatch-plus">+</span>',mediaCallback:'',mediaCache:{},mediaGalleryInitial:[{}],useAjax:false,gallerySwitchStrategy:'replace',inProductList:false,slyOldPriceSelector:'.sly-old-price',oldPriceInitialSelect:false,tierPriceTemplateSelector:'#tier-prices-template',tierPriceBlockSelector:'[data-role="tier-price-block"]',tierPriceTemplate:'',normalPriceLabelSelector:'.normal-price .price-label'},getProduct:function(){var products=this._CalcProducts();return _.isArray(products)?products[0]:null;},_init:function(){if(_.isEmpty(this.options.jsonConfig.images)){this.options.useAjax=true;this._debouncedLoadProductMedia=_.debounce(this._LoadProductMedia.bind(this),500);}
...
return{productId:productId,isInProductView:isInProductView};},_RenderControls:function(){var $widget=this,container=this.element,classes=this.options.classes,chooseText=this.options.jsonConfig.chooseText,singleOptions=[];var colorSelect=null;$widget.optionsMap={};$.each(this.options.jsonConfig.attributes,function(){var item=this,controlLabelId='option-label-'+item.code+'-'+item.id,options=$widget._RenderSwatchOptions(item,controlLabelId),select=$widget._RenderSwatchSelect(item,chooseText),input=$widget._RenderFormInput(item),listLabel='',label='';if($widget.options.onlySwatches&&!$widget.options.jsonSwatchConfig.hasOwnProperty(item.id)){return;}
...
options+select+'</div>'+input+'</div>');$widget.optionsMap[item.id]={};$.each(item.options,function(){if(this.products.length>0){$widget.optionsMap[item.id][this.id]={price:parseInt($widget.options.jsonConfig.optionPrices[this.products[0]].finalPrice.amount,10),products:this.products};}});if(item.options.length==1){$widget.theOption=$widget.element.find('[attribute-id='+item.id+'] .swatch-option')[0];singleOptions.push($widget.element.find('[attribute-id='+item.id+'] .swatch-option')[0]);}else{if(item.code=='color'){colorSelect=$widget.element.find('[attribute-id='+item.id+'] .swatch-option')[0];if(window.defaultColor!==''){$widget.element.find('[attribute-id='+item.id+'] .swatch-option').each(function(){var optionLabel=$(this).attr('option-label');if(optionLabel.indexOf(window.defaultColor)>-1){colorSelect=$(this);}});}}}});container.find('[option-type="1"], [option-type="2"], [option-type="0"], [option-type="3"]').SwatchRendererTooltip();$('.'+classes.moreButton).nextAll().hide();$widget._EventListener();$widget._Rewind(container);if($('body').hasClass('catalog-product-view')){$widget._EmulateSelected($.parseQuery());$widget._EmulateSelected($widget._getSelectedAttributes());if(colorSelect){this.options.oldPriceInitialSelect=true;colorSelect.click();this.options.oldPriceInitialSelect=false;}
$.each(singleOptions,function(){if(typeof this!='undefined'){this.click();}});}},_RenderSwatchOptions:function(config,controlId){var optionConfig=this.options.jsonSwatchConfig[config.id],optionClass=this.options.classes.optionClass,moreLimit=parseInt(this.options.numberToShow,10),moreClass=this.options.classes.moreButton,moreText=this.options.moreButtonText,countAttributes=0,html='';if(!this.options.jsonSwatchConfig.hasOwnProperty(config.id)){return'';}
...
if(type===0){html+='<button type="button" class="'+optionClass+' text" '+attr+'>'+(value?value:label)+'</button type="button">';}else if(type===1){html+='<button type="button" class="'+optionClass+' color" '+attr+'" style="background: '+value+' no-repeat center; background-size: initial;">'+''+'</button type="button">';}else if(type===2){html+='<button type="button" class="'+optionClass+' image" '+attr+'" style="background: url('+value+') no-repeat center; background-size: initial;">'+''+'</button type="button">';}else if(type===3){html+='<button type="button" class="'+optionClass+'" '+attr+'></button type="button">';}else{html+='<button type="button" class="'+optionClass+'" '+attr+'>'+label+'</button type="button">';}});return html;},_RenderSwatchSelect:function(config,chooseText){var html;if(this.options.jsonSwatchConfig.hasOwnProperty(config.id)){return'';}
...
$widget._Rebuild();$widget._UpdatePrice();$widget._loadMedia();$input.trigger('change');},_OnMoreClick:function($this){$this.nextAll().show();$this.blur().remove();},_Rewind:function(controls){controls.find('div[option-id], option[option-id], button[option-id]').removeClass('disabled fictional').removeAttr('disabled aria-disabled');this._DisableElement(controls.find('div[option-empty], option[option-empty]'));},_Rebuild:function(){var $widget=this,controls=$widget.element.find('.'+$widget.options.classes.attributeClass+'[attribute-id]'),selected=controls.filter('[option-selected]');$('#oos-configuration-error').remove();$('#product-addtocart-button').attr('disabled',false);$widget._Rewind(controls);if(selected.size()<=0){return;}
...
$this.find('[option-id]').each(function(){var $element=$(this),option=$element.attr('option-id');if(_.intersection(products,$widget.optionsMap[id][option].products).length<=0){$element.addClass('fictional');}
...
if(_.intersection(products,$widget.optionsMap[id][option].products).length<=0){$widget._DisableElement($element);}});});if(window.oosIds.length){$widget._Disable(controls);}},_DisableElement:function(el){if(el.closest('.swatch-attribute').hasClass('color')){el.addClass('disabled');}else{el.attr('disabled',true).attr('aria-disabled',true).addClass('disabled');}},_preventAddToCart:function(){$('#product-addtocart-button').attr('disabled',true);$('<div generated="true" class="mage-error" id="oos-configuration-error">This selection is out of stock.</div>').insertAfter('.product-options-wrapper');},_Disable:function(controls){var $widget=this;var oosKeys=[];var minSelections=controls.length-1;var selectedCount=controls.filter('[option-selected]').length;var oosConfigurations=[];for(var i=0;i<window.oosIds.length;i++){var attsObject=$widget.options.jsonConfig.index[window.oosIds[i]];var oosConfiguration=[];for(var key in attsObject){if(attsObject.hasOwnProperty(key)){oosConfiguration.push(attsObject[key]);}}
oosConfigurations.push(oosConfiguration);}
var selectedConfiguration=[];controls.each(function(){selectedConfiguration.push($(this).attr('option-selected'));});if(controls.length==1){for(var i=0;i<oosConfigurations.length;i++){if(oosConfigurations[i][0]==selectedConfiguration[0]){$('#product-addtocart-button').attr('disabled',true);}}}
if(selectedCount>=minSelections){for(var i=0;i<oosConfigurations.length;i++){if(_.intersection(oosConfigurations[i],selectedConfiguration).length==minSelections){for(var k=0;k<selectedConfiguration.length;k++){var optionId=oosConfigurations[i][k];if(selectedConfiguration[k]!=optionId){$widget._DisableElement($('[option-id='+optionId+']'));}}}}}
if(selectedCount==controls.length){var shouldReset=false;for(var i=0;i<oosConfigurations.length;i++){if(_.intersection(oosConfigurations[i],selectedConfiguration).length==selectedCount){shouldReset=true;}}
if($('.selected.fictional').length>0){shouldReset=true;}
if(shouldReset){for(var k=1;k<selectedConfiguration.length;k++){$widget._OnClick($('[option-id='+selectedConfiguration[k]+']'),$widget);};}}},_CalcProducts:function($skipAttributeId){var $widget=this,products=[];$widget.element.find('.'+$widget.options.classes.attributeClass+'[option-selected]').each(function(){var id=$(this).attr('attribute-id'),option=$(this).attr('option-selected');if($skipAttributeId!==undefined&&$skipAttributeId===id){return;}
if(!$widget.optionsMap.hasOwnProperty(id)||!$widget.optionsMap[id].hasOwnProperty(option)){return;}
if(products.length===0){products=$widget.optionsMap[id][option].products;}else{products=_.intersection(products,$widget.optionsMap[id][option].products);}});return products;},_UpdatePrice:function(){var $widget=this,$product=$widget.element.parents($widget.options.selectorProduct),$productPrice=$product.find(this.options.selectorProductPrice),options=_.object(_.keys($widget.optionsMap),{}),result,tierPriceHtml;$widget.element.find('.'+$widget.options.classes.attributeClass+'[option-selected]').each(function(){var attributeId=$(this).attr('attribute-id');options[attributeId]=$(this).attr('option-selected');});result=$widget.options.jsonConfig.optionPrices[_.findKey($widget.options.jsonConfig.index,options)];$productPrice.trigger('updatePrice',{'prices':$widget._getPrices(result,$productPrice.priceBox('option').prices)});if(typeof result!='undefined'&&result.oldPrice.amount!==result.finalPrice.amount){$(this.options.slyOldPriceSelector).show();}else{if(this.options.oldPriceInitialSelect===false){$(this.options.slyOldPriceSelector).hide();}}
...
mediaCallData={'product_id':this.getProduct()};mediaCacheKey=JSON.stringify(mediaCallData);if(mediaCacheKey in $widget.options.mediaCache){$widget._XhrKiller();$widget._EnableProductMediaLoader($this);mediaSuccessCallback($widget.options.mediaCache[mediaCacheKey]);}else{mediaCallData.isAjax=true;$widget._XhrKiller();$widget._EnableProductMediaLoader($this);$widget.xhr=$.get($widget.options.mediaCallback,mediaCallData,mediaSuccessCallback,'json').done(function(){$widget._XhrKiller();});}},_EnableProductMediaLoader:function($this){var $widget=this;if($('body.catalog-product-view').size()>0){$this.parents('.column.main').find('.photo.image').addClass($widget.options.classes.loader);}else{$this.parents('.product-item-info').find('.product-image-photo').addClass($widget.options.classes.loader);}},_DisableProductMediaLoader:function($this){var $widget=this;if($('body.catalog-product-view').size()>0){$this.parents('.column.main').find('.photo.image').removeClass($widget.options.classes.loader);}else{$this.parents('.product-item-info').find('.product-image-photo').removeClass($widget.options.classes.loader);}},_ProductMediaCallback:function($this,response,isInProductView){var $main=isInProductView?$this.parents('.column.main'):$this.parents('.product-item-info'),$widget=this,images=[],caption=$('.page-title').text().trim()+' shown in '+$('.swatch-attribute.color .swatch-attribute-selected-option').text().trim(),support=function(e){return e.hasOwnProperty('large')&&e.hasOwnProperty('medium')&&e.hasOwnProperty('small');};if(_.size($widget)<1||!support(response)){this.updateBaseImage(this.options.mediaGalleryInitial,$main,isInProductView);return;}
images.push({full:response.large,img:response.medium,thumb:response.small,isMain:true,caption:caption});if(response.hasOwnProperty('gallery')){var _index=1;$.each(response.gallery,function(){if(!support(this)||response.large===this.large){return;}
images.push({full:this.large,img:this.medium,thumb:this.small,caption:caption+' alternate view '+_index++});});}
