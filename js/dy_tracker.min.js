...
exports.domReady=domReady;})(window,document);(function(DY,exports,doc){"use strict";function DynamicYield_Tracking(){exports.domReady(function(){this.onLoad();this.ajaxEvent(XMLHttpRequest);}.bind(this));}
...
xhr.open('GET','/dyIntegration/storage/index?requestId="'+getUniqueId()+'"');xhr.onload=function(){if(xhr.status===200){var response=JSON.parse(xhr.responseText);if(response.events){response.events.forEach(function(event){try{DY.API('event',event.properties);}catch(e){}});}}};xhr.send();}catch(e){}};DynamicYield_Tracking.prototype.syncCartEvent=function(){try{if(!sessionStorage.isNewSession){sessionStorage.isNewSession=true;if(window.XMLHttpRequest){var xhr=new XMLHttpRequest();}else{var xhr=new ActiveXObject("Microsoft.XMLHTTP");}
xhr.open('GET','/dyIntegration/synccart/index');xhr.onload=function(){if(xhr.status===200){var session=JSON.parse(xhr.responseText);if(session.sync_cart==false){var eventData={name:session.eventData.name,properties:session.eventData.properties};try{DY.API('event',eventData);}catch(e){}}}};xhr.send();}}catch(e){}};function getUniqueId(){return parseInt(Date.now()+Math.random());}
...
var targetHeader=DY_SETTINGS.headerName||'dy-event-data';if(dy_headers[targetHeader]){var json=JSON.parse(headers[targetHeader]);try{DY.API('event',json);}catch(e){}}}catch(e){}}
...
switcherTrigger.addEventListener('click',function(event){DynamicYield_Tracking.prototype.onSortChange(event,true);},false);},1,100,100);}else if(type==='product'){DYO.waitForElement(DY_SETTINGS.eventSelectors.product_page_attribute_trigger,function(){var dropDownTrigger=document.querySelectorAll(DY_SETTINGS.eventSelectors.product_page_attribute_trigger);for(var s=0;s<dropDownTrigger.length;s++){dropDownTrigger[s].onchange=function(event){DynamicYield_Tracking.prototype.onProductAttributeSelectChange(event);}}},1,100,100);DYO.waitForElement(DY_SETTINGS.eventSelectors.product_page_custom_option_trigger,function(){var customOptions=document.querySelectorAll(DY_SETTINGS.eventSelectors.product_page_custom_option_trigger);for(var s=0;s<customOptions.length;s++){customOptions[s].onclick=function(event){DynamicYield_Tracking.prototype.onProductCustomOptionChange(event);}}},1,100,100);DYO.waitForElement(DY_SETTINGS.eventSelectors.product_page_swatch_trigger,function(element){var swatchTrigger=document.querySelectorAll(DY_SETTINGS.eventSelectors.product_page_swatch_trigger);for(var s=0;s<swatchTrigger.length;s++){swatchTrigger[s].onclick=function(event){DynamicYield_Tracking.prototype.onProductSwatchClick(event);}}},1,100,100);}};DynamicYield_Tracking.prototype.onLayeredNavClick=function(event){var self=event.currentTarget;var filterString=false;var name=this.applySelectors(self,DY_CUSTOM_STRUCTURE.category_page_filters_type);var value=this.applySelectors(self,DY_CUSTOM_STRUCTURE.category_page_filters_price_value)||this.applySelectors(self,DY_CUSTOM_STRUCTURE.category_page_filters_regular_value)||this.applySelectors(self,DY_CUSTOM_STRUCTURE.category_page_filters_swatch_value)||this.applySelectors(self,DY_CUSTOM_STRUCTURE.category_page_filters_swatch_image_value);if(value&&value.toString().match(/[a-z]/i)){filterString=true;}
