requirejs(['algoliaBundle','Magento_Catalog/js/price-utils'],function(algoliaBundle,priceUtils){algoliaBundle.$(function($){if(window.innerWidth>=992){algoliaConfig.autocomplete.enabled=false;}
if(!algoliaConfig.instant.enabled||!(algoliaConfig.isSearchPage||!algoliaConfig.autocomplete.enabled)){return;}
...
return options;}});require(['jquery','algoliaBundle','domReady!',],function($){jQuery('#algolia-searchbox').find('.algolia-search-input').keydown(function(e){if(e.keyCode==13){e.preventDefault();return false;}});jQuery('.algolia-search-input').find('#nav-search-field').keydown(function(e){if(e.keyCode==13){e.preventDefault();return false;}});var toggleActiveClassForMobile=function(isActive){$('#algolia-searchbox').toggleClass('active',isActive);};var toggleActiveClassForDesktop=function(isActive){$('.ais-SearchBox-input').toggleClass('active',isActive);$('.algolia-instant-results-wrapper').toggleClass('active',isActive);};var urlParams=new URLSearchParams(window.location.search);var query=urlParams.get('q');if(query&&query!=='__empty__'&&window.innerWidth>=992){window.setTimeout(function(){toggleActiveClassForDesktop(true);},2000);}
$(document).on('keyup','#search',function(){var isSearching=!!($(this).val()!=='');toggleActiveClassForMobile(isSearching);});$(document).on('keyup','.ais-SearchBox-input',function(){var isSearching=!!($(this).val()!=='');toggleActiveClassForDesktop(isSearching);});$(document).on('click','.clear-query-autocomplete',function(){toggleActiveClassForMobile(false);$('#search').val('');});$(document).on('click','.ais-SearchBox-reset',function(){toggleActiveClassForDesktop(false);$('.ais-SearchBox-input').val('');});$(document).on('click','.ais-CurrentRefinements-categoryLabel',function(){$(this).parent().find('.ais-CurrentRefinements-delete').click();});$(document).on('click','a[href="#mainSearch"]',function(){$('#search').val('');$('.aa-dropdown-menu').hide();window.setTimeout(function(){$('#search').focus();},300);});});