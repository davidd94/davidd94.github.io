define([
    ...
    ko.bindingHandlers.price = {
        init: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
            var $element = $(element);
            var s = '';
            if(bindingContext.$data.isSwatch && parseFloat(bindingContext.$data.minPrice) != parseFloat(bindingContext.$data.maxPrice)) {
                s = '+';
            }
    ...
    return {
        ...
        processApplyBinding: function (data) {
            ...
            $(this.placeholderSelector).html(wrapper);

            data.wrapClassName = '';
            
            for (var i = 0; i < data.indices.length; i++){
                data.wrapClassName += ' searchindex-' + data.indices[i].identifier + '-' + data.indices[i].totalItems;
            }
        ...
        ensurePosition: function () {
            ...
            var left = $('.header.content').position().left + parseInt($('.header.content').css('paddingLeft'), 10);
            var top = position.top + parseInt($input.css('marginTop'), 10);
            $(this.placeholderSelector)
                .css('top', $input.outerHeight() + top)
                .css('left', left)
                .css('width', $('.header.content').width());
        }
    }
});
