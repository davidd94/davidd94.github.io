(function ($) {
    ...
    var ella = {
        ...
        checkItemsInDropdownCart: function () {
          ...
          if (miniProductList.length == 0) { miniProductList = dropdownCart.find('.mini-products-list')}
        ...
        doUpdateDropdownCart: function (cart) {
            var template = '<li class="item" id="cart-item-{ID}"><a href="{URL}" title="{TITLE}" class="product-image"><img src="{IMAGE}" alt="{TITLE}"></a><div class="product-details"><a class="product-name" href="{URL}">{TITLE}</a>{OPTIONS}<div class="option"><small>{VARIANT}</small></div><div class="cart-collateral">	<span class="price">{PRICE}</span><span class="qtt" data-variant-id="{ID}"><a href="#" data-minus-quantity="" class="minus button"></a> <span class="val">{QUANTITY}</span> <a href="#" data-plus-quantity="" class="plus button"></a> <a href="javascript:void(0)"  class="btn-remove"> <svg aria-hidden="true" data-prefix="fal" data-icon="times" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" class="svg-inline--fa fa-times fa-w-10 fa-2x"><path fill="currentColor" d="M193.94 256L296.5 153.44l21.15-21.15c3.12-3.12 3.12-8.19 0-11.31l-22.63-22.63c-3.12-3.12-8.19-3.12-11.31 0L160 222.06 36.29 98.34c-3.12-3.12-8.19-3.12-11.31 0L2.34 120.97c-3.12 3.12-3.12 8.19 0 11.31L126.06 256 2.34 379.71c-3.12 3.12-3.12 8.19 0 11.31l22.63 22.63c3.12 3.12 8.19 3.12 11.31 0L160 289.94 262.56 392.5l21.15 21.15c3.12 3.12 8.19 3.12 11.31 0l22.63-22.63c3.12-3.12 3.12-8.19 0-11.31L193.94 256z" class=""></path></svg> </a></span></div></div></li>';
          ...
          if (miniProductList.length == 0 && $('.translate-header #dropdown-cart .has-items').length == 0) {
            $('.translate-header #dropdown-cart .no-items').remove();
            $('.translate-header #dropdown-cart').append('<div class="has-items"><ol class="mini-products-list"></ol></div>')
            $('.translate-header #dropdown-cart').append('<div class="summary"> <div class="total"> <span class="label"> <span> Total: </span> </span> <span class="price">' + Shopify.formatMoney(cart.total_price, window.money_format) + '</span> </div></div><div class="actions"> <a class="btn btn-view-cart" href="/cart"> View Cart </a> <button class="btn btn-checkout" onclick="window.location=\'/checkout\'"> Check Out </button> </div>')
            miniProductList = $('ol.mini-products-list')
          }

            miniProductList.html('');
          
          $('.header-top .cart-icon .priced,  #new-cart .total + .total .price, #dropdown-cart .total .price').text(Shopify.formatMoney(cart.total_price, window.money_format))
            ...
            if (cart.item_count > 0) {
                for (var i = 0; i < cart.items.length; i++) {
                    ...
                    if (cart.items[i].properties) {
                      Object.keys(cart.items[i].properties).forEach(function(key) {
                        options += '<div class="option">' + key + ': ' + cart.items[i].properties[key] + '</div>'
                      })
                    }
                    ...
                    item = item.replace(/\{OPTIONS\}/g, options);
        ...
        changeQuantityAddToCart: function () {
            ...
            doc.on('click', buttonSlt, function (e) {
                ...
                $(input).text(val);
                variantId = self.parent().attr('data-variant-id')
                obj = {}
                obj[variantId] = val
        ...
        initProductMoreview: function (productMoreview) {
            $('.slider-for').on('init', function(event, slick){
                function hidebg(){
                    $('.slider-for').css("background-image", "none")
                };
                setTimeout(hidebg, 1000);
});
